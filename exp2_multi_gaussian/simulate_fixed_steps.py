...
from helper import getoptJ, getnoptJ, getJ, score_log_gmm as gradfunc, fisher_info_pi as estimate_fisher
...
def simulate_chain(mm, hh, K, fisher, means, weights, covs, inv_covs):
    # Setup
    d = len(means[0])
    N = 10                                              # number of descretized chains
    K_N = int(K / N)                                    # number of steps per chain
    ...
    for nn in range(N):
        Y_his = np.zeros((d, K_N))
        Y_his[:, 0] = Y
        for kk in range(K_N - 1):
...
def main(M,h,K,means=None,weights=None,covs=None,path=None):
    ...
    with Parallel(n_jobs=num_cores, backend="multiprocessing") as parallel:
        results = parallel(delayed(simulate_chain)(mm, h, K, fisher, means, weights, covs, inv_covs) for mm in tqdm(range(M)))
...
if __name__ == "__main__":
    ...
    main(M=args.M, h=args.h, K=args.K, path=args.path)