...
"""
This script is used to run the simulation for the 4-D Gaussian with parallel chains,
compare the performance of reversible and irreversible Langevin using adaptive Fisher information matrix.
"""
...
def main(M,h,T,sigma_true,path):
    ...
    Yrmall2 = np.zeros((d, K, M))
    Yoptall1 = np.zeros((d, K, M))
    Yoptall2 = np.zeros((d, K, M))
    ...
    def simulate_chain(mm):
        ...
        Yrm1[:, 0] = initcond
        for kk in range(K - 1):
            gradeval = gradfunc(Yrm1[:, kk], fisher)
            if kk == 0:
                FIM_inv = initialize_A1(gradeval)
            else:
                FIM_inv = iterate_An(FIM_inv, gradeval, kk)
            Yrm1[:, kk + 1] = Yrm1[:, kk] + h / 2 * (FIM_inv) @ gradeval + np.linalg.cholesky(h * FIM_inv) @ np.random.randn(d)

        # RM Langevin constant, optimal preconditioner
        Yrm2 = np.zeros((d, K))
        Yrm2[:, 0] = initcond
        for kk in range(K - 1):
            gradeval = gradfunc(Yrm2[:, kk], fisher)
            Yrm2[:, kk + 1] = Yrm2[:, kk] + h / 2 * (cov_true) @ gradeval + np.linalg.cholesky(h * cov_true) @ np.random.randn(d)
        ...
        return Y, Yrm1, Yrm2, Yopt1, Yopt2  # list of shape [11 arrays of shape (d, K)]
    ...
    Yall, Yrm1all, Yrm2all, Yopt1all, Yopt2all = [np.array(x) for x in zip(*results)]
    Yall = Yall.transpose(1, 2, 0)
    Yrm1all = Yrm1all.transpose(1, 2, 0)
    Yrm2all = Yrm2all.transpose(1, 2, 0)
    Yopt1all = Yopt1all.transpose(1, 2, 0)
    Yopt2all = Yopt2all.transpose(1, 2, 0)
    print(Yall.shape)
    ...
    Y_list = [Yall, Yrm1all, Yrm2all, Yopt1all, Yopt2all]
    methods = ['Standard', 'RM_esti', 'RM_true', 'opt_esti', 'opt_true']
    ...
    plt.tight_layout()
    plt.title(f'4-D Gaussian with covariance {np.diag(cov_true)} for estimation and true FIM')
    ...
    plt.suptitle(f'4-D Gaussian trajectory plot with estimation and true FIM \
