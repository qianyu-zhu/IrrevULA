...
def main(M,h,T):
    ...
    J_nopt1, J_nopt2, J_nopt3, J_nopt4, J_nopt5 = [getnoptJ(fisher) for _ in range(5)]
    J_opt1, J_opt2, J_opt3, J_opt4, J_opt5 = [getoptJ(fisher) for _ in range(5)]
    ...
    for J in [J_nopt1, J_nopt2, J_nopt3, J_nopt4, J_nopt5]:
        print('(I+J)@fisher', np.round(np.linalg.eigvals((np.eye(d) + J)@fisher), decimals=3))
    print('spectrum of opt J')
    for J in [J_opt1, J_opt2, J_opt3, J_opt4, J_opt5]:
        print('(I+J)@fisher', np.round(np.linalg.eigvals((np.eye(d) + J)@fisher), decimals=3))
    ...
    Ynopt1all, Ynopt2all, Ynopt3all, Ynopt4all, Ynopt5all = [np.zeros((d, K, M)) for _ in range(5)]
    Yopt1all, Yopt2all, Yopt3all, Yopt4all, Yopt5all = [np.zeros((d, K, M)) for _ in range(5)]
