...
from gradfunc import gradlogpos_irrwriem_new
    ...
    def run_chain(ii, subsample_rate):
        ...
        for nn in range(N):
            ...
            for jj in range(num_steps//N):
                grad = gradlogpos_irrwriem_new(W_ir[:, jj], X, J, lambda_)
                W_ir_sq = np.reshape(W_ir[:, jj], (d, d))
                # noise = np.random.randn(d)
                # G = sqrtm(np.eye(d) + W_ir_sq.T @ W_ir_sq)
                # noise_term = np.reshape(np.real(G @ noise), -1)
                noise_vec = np.random.randn(d, d) @ np.real(sqrtm(np.eye(d) + W_ir_sq.T @ W_ir_sq))
                noise_term = np.reshape(noise_vec, d**2)
                W_ir[:, jj + 1] = W_ir[:, jj] + dt * grad / 2 + np.sqrt(dt) * noise_term
