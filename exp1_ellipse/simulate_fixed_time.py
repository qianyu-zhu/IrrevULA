...
def stat_func(Y):
    ...
    return np.array([np.mean(np.sum(np.abs(Y), axis=0)), np.mean(np.sum(Y**2, axis=0)), \
                     ...
                     np.sum(np.max(Y,axis=0) > 1)/len(Y[0, :]), np.sum(np.max(Y,axis=0) > 2)/len(Y[0, :]), \
                     np.sum(np.max(Y,axis=0) > 8)/len(Y[3, :]), np.sum(np.max(Y,axis=0) > 16)/len(Y[3, :])])

def simulate_chain(mm, h, T, sigma_true):
    # Setup
    d = 4
...
def main(M,hh,T,sigma_true,path):
    ...
    with Parallel(n_jobs=num_cores, backend="multiprocessing") as parallel:
        results = parallel(delayed(simulate_chain)(mm, hh, T, sigma_true) for mm in tqdm(range(M)))
